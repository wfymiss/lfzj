...
public class UnitListPopup extends PopupWindow {
    ...
    public UnitListPopup(Context mcontext){
    ...
    private void init(Context ncontext) {
       ...
       adapter.setTitleItemListener(new BuildingListTitleAdapter.TitleItemListener() {
           ...
           public void titleItemClick(int position, String building, String type_id) {
               EventBus.getDefault().post(new BuildingListEvent(building,type_id,"2"));
               RxBus.getDefault().post(new BuildingListEvent(building,type_id,"2"));
               dismiss();
           }
       });
    }
    ...
    private void activityCategory() {
        SharedPreferences spf=context.getSharedPreferences("unit", Context.MODE_PRIVATE);
        String category=spf.getString("unit_id","");
